...
namespace rmi
{
  ...
  {
    // std::cout << "[sendRequest]: request :" << msg_out << std::endl
    // << std::flush; // debug
  ...
  bool RMIDriver::parseCommunication(std::string msg_in)
  {
    ...
    std::cout << "\033[1;36m[parseCommunication]: response :" << msg_in << "\033[0m" << std::endl
              << std::flush;
  ...
  {
    std::cout << "\033[1;36m[parseGetStatus]: response :" << msg_in << "\033[0m" << std::endl
              << std::flush;
